...
# how similar the regions of genome are that have mutations
...
expanded_data <- data.frame(
  ...
  mutation_ID = rep(data2$mutation_ID, lengths(data2$Strain)), # change to data2 if looking at filters, data if not
...
strain_categories <- data.frame(
  ...
  Category = case_when(
    grepl("^JOG", rownames(transposed_matrix)) ~ "JOG (Cocktail)", # Cocktail/JOG
    grepl("^MAT", rownames(transposed_matrix)) ~ "MAT (Cocktail)", # Cocktail/MAT
    grepl("^NW", rownames(transposed_matrix))  ~ "NW (Acetic)", # Acetic/NW
    grepl("^PP", rownames(transposed_matrix))  ~ "PP (Formic)", # Formic/PP
    grepl("^WS", rownames(transposed_matrix))  ~ "WS (Acetic)" # Acetic?WS
  )
)
...
# 7. Plot the heatmap with clustering and colored strain - but removed so simplicity when viewing figure
pheatmap(
  transposed_matrix,                 
  color = c("white", "black"),       
  ...
  cluster_rows = TRUE,               
  cluster_cols = FALSE,               
  legend = FALSE,                     # Remove color scale legend (0 and 1)
  annotation_legend = FALSE,         # Remove annotation legend (strain categories)
  breaks = c(-0.1, 0.5, 1), 
  fontsize_row = 5,                  
  fontsize_col = 3,
  show_colnames = FALSE,              
