# 20JAN25 - Coverage Plots
# This code makes 97 coverage panels
# Each coverage panel represents each evolved strain and the ancestor
# There will be 7 plots in each strain to represent each chromosome
# This will be most likely used for the supplmentary material

# Key steps

# 1) Set appropraite working directory
# 2) Read in data from each txt file and make into accessible format
# 3) Define window size
# 4) Summerize the data into the window size to create compressed txt
# 5) Combine all compressed txt files 
...
# 1) Get and set the appropriate working directory
getwd()
...
# Load data - read all compressed txt files one at a time
coverage_data <- read.table("JOG3.txt", header = FALSE) # change this to appropriate strain
head(coverage_data)
...
# Making a note of the median coverage
...
# 6) Combine all individual plots into one big pdf files
