...
new_dataset <- interactive_coverage_datagene %>%
  ...
  mutate(strain = Different_Columns, 
         gene = case_when(
           ...
           Gene.ID == "unknown / unknown" ~ paste0("unknown / unknown-", End.Position),
           grepl(" / unknown$", Gene.ID) ~ paste0(Gene.ID, "-", End.Position),  # Gene / unknown → Gene / unknown-End.Position
           grepl("^unknown / ", Gene.ID) ~ Gene.ID,  # unknown / gene 
           TRUE ~ Gene.ID  # Keep other Gene.ID values unchanged
         )) %>%  
  # Select relevant columns to keep in the final dataset
  select(strain, gene)
...
# Choose the appropriate one (filter_dist)
...
# mat <- as.matrix(snp_general_dist) # Regions with mutations
# mat <- as.matrix(snp_gene_dist) # Genes with mutations
...
# Add significance labeling
fixed_effects$sig <- ifelse(fixed_effects$p.value < 0.05, "Significant", "Not Significant")

# Create the plot with black text
ggplot(fixed_effects, aes(x = term, y = estimate, color = sig)) +
  geom_point(size = 4) +  # Increase the size of the points
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.15) +  # Narrower error bars
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  coord_flip() +
  scale_color_manual(values = c("black", "red")) +
  theme_minimal(base_size = 14) +  # Increase base font size for readability
  theme(
    axis.text.y = element_text(size = 12, color = "black"),  # Adjust y-axis label size and color
    axis.title = element_text(size = 12, color = "black"),  # Adjust axis title size and color
    plot.title = element_text(hjust = 0.5, size = 12, face = "bold", color = "black"),  # Title adjustments with black text
    legend.position = "none",  # Hide the legend as it's redundant with color
    plot.margin = margin(1, 10, 10, 10)  # Adjust margins to reduce white space
  ) +
  labs(
    title = "Non-synonomous mutations",
    x = NULL, y = "Coefficient estimates and 95% confidence intervals"  # Remove x-axis label
  )
