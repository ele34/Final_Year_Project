# 26MAR25 - Interactive Coverage Table for Genome

# Read in file
getwd()
# Load original dataset (keep decimal values)
df <- read.csv("Table_CRC_allregions56.csv", check.names = FALSE, stringsAsFactors = FALSE)  # Preserve column names
...
write.csv(df, "filteredall_ch56.csv", row.names = FALSE)

# Load necessary library
library(dplyr)

# Read in the original table and important genes table
original_table <- read.csv("Table_CRC_allregions56.csv")  
important_genes <- read.csv("filteredall_ch56.csv")  
...
write.csv(filtered_table, "filtered_genesch56all.csv", row.names = FALSE)
...
write.csv(plusminus_coverage_data, "interactive_coverage_data_all.csv", row.names = FALSE)
